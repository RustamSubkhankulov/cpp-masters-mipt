...
private:
  ...
  std::size_t CountAliveNeighbors(const std::size_t row,
    ...
    const std::size_t column_end =
      (column + 1U < kWidth) ? column + 1U : kWidth - 1U;

    for (std::size_t neighbor_row = row_begin; neighbor_row <= row_end;
         ++neighbor_row) {
      for (std::size_t neighbor_column = column_begin;
           neighbor_column <= column_end; ++neighbor_column) {
