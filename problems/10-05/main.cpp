...
to random, the growth is close to quadratic at small loads and then becomes
almost linear when the table is significantly filled. If a hash function
produces stronger clustering, its curve becomes steeper because similar input
strings are sent to the same buckets more often.
...
The worst hash function in the tested set is DEKHash, which produced 79230
collisions. Two other clearly weak functions are ELFHash and PJWHash, each with
73023 collisions. These values are much higher than the reference level of
48720.1, which means that on this dataset these functions distributed values
across buckets much less uniformly and caused much stronger clustering.

The full ranking from best to worst is exactly the following: DJBHash — 40729,
RSHash — 42366, SDBMHash — 43331, BKDRHash — 47345, JSHash — 47997, APHash —
48725, ELFHash — 73023, PJWHash — 73023, DEKHash — 79230. The graph was built
for all nine functions from the dataset.
*/
...
HashValue PJWHash(std::string_view text) {
  constexpr std::size_t kBitsInUnsignedInt =
    std::numeric_limits<HashValue>::digits;
  constexpr std::size_t kThreeQuarters = (kBitsInUnsignedInt * 3U) / 4U;
  constexpr std::size_t kOneEighth = kBitsInUnsignedInt / 8U;
  constexpr HashValue kHighBits = 0xFFFFFFFFU
...
HashValue SDBMHash(std::string_view text) {
  ...
  for (unsigned char character : text) {
    hash =
      static_cast<HashValue>(character) + (hash << 6U) + (hash << 16U) - hash;
  }
...
HashValue APHash(std::string_view text) {
  ...
  for (std::size_t index = 0; index < text.size(); ++index) {
    const HashValue character =
      static_cast<HashValue>(static_cast<unsigned char>(text[index]));
...
MeasureCollisionCurves(const std::vector<std::string>& strings,
                       std::size_t bucket_count, std::size_t sample_step) {
  if (bucket_count == 0U) {
  ...
  for (std::size_t string_index = 0; string_index < strings.size();
       ++string_index) {
    const std::string_view text = strings[string_index];

    for (std::size_t hash_index = 0; hash_index < kHashCatalog.size();
         ++hash_index) {
      const std::size_t bucket = static_cast<std::size_t>(
        kHashCatalog[hash_index].function(text) % bucket_count);
...
std::vector<RankingEntry>
RankByFinalCollisions(const std::vector<CollisionPoint>& points) {
  if (points.empty()) {
    return {};
  }
  ...
  for (std::size_t index = 0; index < kHashCatalog.size(); ++index) {
    ranking.push_back(
      RankingEntry{kHashCatalog[index].name, points.back().collisions[index]});
  }
...
double ExpectedRandomCollisions(std::size_t string_count,
  ...
  const double one_minus_inverse_bucket_count =
    1.0 - 1.0 / bucket_count_as_double;
  const double occupied_expectation =
    bucket_count_as_double *
    (1.0 - std::pow(one_minus_inverse_bucket_count, string_count_as_double));

  return string_count_as_double - occupied_expectation;
}

void WriteCollisionCsv(const std::filesystem::path& file_path,
...
void WriteSummary(const std::filesystem::path& file_path,
                  const std::vector<CollisionPoint>& points,
                  std::size_t bucket_count) {
  if (points.empty()) {
  ...
  const double expected_random_collisions =
    ExpectedRandomCollisions(final_string_count, bucket_count);
  ...
  output << "Expected random collisions at final point: "
         << expected_random_collisions << "\n\n";
  output << "Best function: " << ranking.front().name << " with "
         << ranking.front().collisions << " collisions\n";
  output << "Worst function: " << ranking.back().name << " with "
         << ranking.back().collisions << " collisions\n\n";
...
void WriteGnuplotScript(const std::filesystem::path& file_path,
  ...
  for (std::size_t index = 0; index < kHashCatalog.size(); ++index) {
    output << "    '" << csv_file_path.filename().string()
           << "' using 1:" << (index + 2U) << " with lines lw 2 title '"
           << kHashCatalog[index].name << "'";
...
ResearchArtifacts RunResearch(const std::filesystem::path& output_directory,
  ...
  const std::vector<std::string> strings =
    MakeSequentialDecimalStrings(string_count);
  std::vector<CollisionPoint> points =
    MeasureCollisionCurves(strings, bucket_count, sample_step);
  std::vector<RankingEntry> ranking = RankByFinalCollisions(points);

  const std::filesystem::path csv_file_path =
    output_directory / "hash_collisions_decimal.csv";
  const std::filesystem::path summary_file_path =
    output_directory / "hash_summary_decimal.txt";
  const std::filesystem::path gnuplot_file_path =
    output_directory / "plot_hash_collisions_decimal.gnuplot";
  const std::filesystem::path image_file_path =
    output_directory / "hash_collisions_decimal.png";
  ...
  return ResearchArtifacts{
    csv_file_path,   summary_file_path, gnuplot_file_path,
    image_file_path, std::move(points), std::move(ranking),
  };
}
...
TEST(HashResearchTest, MiniDecimalDatasetCollisionCountsAreStable) {
  ...
  const std::vector<std::string> strings =
    hash_research::MakeSequentialDecimalStrings(kStringCount);
  ...
  constexpr std::array<std::size_t, hash_research::kHashCatalog.size()>
    expected = {
      9U, 6U, 8U, 8U, 8U, 1U, 1U, 6U, 4U,
    };
...
TEST(HashResearchTest, DefaultResearchIdentifiesBestAndWorstFunctions) {
  ...
  const auto artifacts = hash_research::RunResearch(
    std::filesystem::current_path() / "hash_research_output", kStringCount,
    kBucketCount, kSampleStep);
