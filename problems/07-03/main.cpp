...
2. The `if` statement selects one of two normal paths:
   - `save(RunStatus::kSuccess, person.id())`
   - `save(RunStatus::kFailure, person.id())`
...
enum class RunStatus { kSuccess = 0, kFailure = 1 };

enum class ThrowPoint { kNone, kName, kGrade, kSalary, kID };
...
public:
  ...
  [[nodiscard]] std::string const& name() const {
    if (throw_point_ == ThrowPoint::kName) {
      throw NameError();
    }
    return name_;
  }
  ...
  [[nodiscard]] int grade() const {
    if (throw_point_ == ThrowPoint::kGrade) {
      throw GradeError();
    }
    return grade_;
  }
  ...
  [[nodiscard]] int salary() const {
    if (throw_point_ == ThrowPoint::kSalary) {
      throw SalaryError();
    }
    return salary_;
  }
  ...
  [[nodiscard]] int id() const {
    if (throw_point_ == ThrowPoint::kID) {
      throw IdError();
    }
    return id_;
  }
...
[[nodiscard]] std::string_view ToString(RunStatus status) {
  switch (status) {
  case RunStatus::kSuccess:
    return "kSuccess";
  case RunStatus::kFailure:
    return "kFailure";
  }
...
void save(RunStatus status, int id) {
...
void test(Person const& person) {
  ...
  if (person.grade() == ExcellentGrade() ||
      person.salary() > HighSalaryThreshold()) {
    save(RunStatus::kSuccess, person.id());
  } else {
    save(RunStatus::kFailure, person.id());
  }
...
[[nodiscard]] std::string DemonstrateShortCircuitAvoidsSalary() {
  Person const person("Dave", ExcellentGrade(), 0, 4, ThrowPoint::kSalary);
  return CaptureCout([&person]() { test(person); });
}

void DemonstrateNameException() {
  Person const person("Eve", ExcellentGrade(), 1, 5, ThrowPoint::kName);
  test(person);
}

void DemonstrateGradeException() {
  Person const person("Frank", ExcellentGrade(), 1, 6, ThrowPoint::kGrade);
  test(person);
}

void DemonstrateSalaryException() {
  Person const person("Grace", 9, HighSalaryThreshold() + 1, 7,
                      ThrowPoint::kSalary);
  test(person);
}

void DemonstrateIdException() {
  Person const person("Heidi", ExcellentGrade(), 1, 8, ThrowPoint::kID);
  test(person);
}
