...
public:
  enum class SearchPolicy { FirstFit, BestFit };

  explicit Allocator(std::size_t size,
                     SearchPolicy policy = SearchPolicy::FirstFit)
    : m_size(size)
    , m_policy(policy) {
    ...
    m_begin = static_cast<std::byte*>(
      ::operator new(m_size, std::align_val_t(kAlignment)));
...
private:
  ...
  adjusted_payload_size(std::size_t requested_size) {
  ...
  [[nodiscard]] std::pair<Node*, Node*> find(std::size_t size) const {
    return m_policy == SearchPolicy::FirstFit ? find_first(size)
                                              : find_best(size);
  }
...
TEST(AllocatorTests, ReusesFreedBlock) {
  ...
  EXPECT_EQ(reinterpret_cast<std::uintptr_t>(first) % alignof(std::max_align_t),
            0U);
...
void run_allocator_benchmark(benchmark::State& state,
                             Allocator::SearchPolicy policy) {
  ...
  for (auto _ : state) {
    ...
    for (std::size_t index = 0U; index < workload.initial_sizes.size();
         ++index) {
