...
Rational ReduceRatio(const std::size_t numerator,
                     const std::size_t denominator) {
  const std::size_t divisor = std::gcd(numerator, denominator);
  return {numerator / divisor, denominator / divisor};
}

std::vector<CapacityChange>
TrackVectorCapacityChanges(const std::size_t required_change_count) {
...
std::vector<Rational>
BuildVectorGrowthRatios(const std::vector<CapacityChange>& changes) {
  ...
  for (const CapacityChange& change : changes) {
    if (change.capacity_before != 0U) {
      ratios.push_back(
        ReduceRatio(change.capacity_after, change.capacity_before));
    }
  }
...
Rational FindMostFrequentRatio(const std::vector<Rational>& ratios,
...
std::size_t FindMostFrequentElementAllocationCount(
  ...
  for (const AllocationRecord& record : records) {
    ...
    if (count > best_count ||
        (count == best_count && record.element_count > best_allocation)) {
      best_count = count;
...
TrackDequeAllocations(const std::size_t element_count) {
...
std::size_t DetectDequePageSizeInElements(const std::size_t element_count) {
  const std::vector<AllocationRecord> records =
    TrackDequeAllocations<T>(element_count);
...
TrackDequeBackAddresses(const std::size_t element_count) {
  std::deque<T> values;
...
BuildContiguousRunLengths(const std::vector<std::uintptr_t>& addresses) {
  ...
  for (std::size_t index = 1; index < addresses.size(); ++index) {
    const std::uintptr_t expected_next_address =
      addresses[index - 1U] + sizeof(T);
...
void ExpectStableDequeAddressesAfterPushBack(
  const std::size_t prefix_size, const std::size_t extra_insertions) {
  std::deque<T> values;
...
TEST(VectorCapacityInvestigation,
     CapacityChangesAreObservedOnlyWhenStorageIsFull) {
  const std::vector<CapacityChange> changes =
...
TEST(VectorCapacityInvestigation,
     StableGrowthFactorIsDerivedFromTheObservedCapacities) {
  const std::vector<CapacityChange> changes =
    TrackVectorCapacityChanges(kTrackedVectorCapacityChanges);
  const std::vector<Rational> ratios = BuildVectorGrowthRatios(changes);
  const std::size_t warmup_count =
    std::min<std::size_t>(3U, ratios.size() / 2U);
  const Rational growth_factor =
    DetectVectorGrowthFactor(kTrackedVectorCapacityChanges);
...
TEST(DequePageInvestigation, RepeatedElementAllocationsRevealPageSize) {
  const std::vector<AllocationRecord> records =
    TrackDequeAllocations<DequeValue>(kTrackedDequeElements);
  ...
  const std::size_t page_size_in_bytes =
    DetectDequePageSizeInBytes<DequeValue>(kTrackedDequeElements);
...
TEST(DequePageInvestigation,
     RecordedAddressesContainAContiguousSegmentAtLeastOnePageLong) {
  const std::size_t page_size_in_elements =
...
TEST(DequePageInvestigation,
     ExistingElementAddressesRemainStableAfterFurtherInsertions) {
  ExpectStableDequeAddressesAfterPushBack<DequeValue>(kTrackedDequePrefix,
