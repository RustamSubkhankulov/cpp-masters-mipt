...
protected:
  ...
  static T* get(void* address) noexcept {
    static_assert(std::is_same_v<T, std::byte> || std::is_same_v<T, Header> ||
...
public:
  explicit LinearAllocator(std::size_t capacity)
    : m_capacity(capacity)
    , m_memory(
        ::operator new(m_capacity, std::align_val_t(kDefaultAlignment))) {}
...
public:
  ...
  void* allocate(std::size_t size,
    ...
    const std::size_t actual_alignment = std::max(alignment, alignof(Header));
    ...
    auto* header = get<Header>(get<std::byte>(aligned) - sizeof(Header));

    ::new (static_cast<void*>(header)) Header{static_cast<std::size_t>(
      get<std::byte>(aligned) - (get<std::byte>(m_memory) + m_offset))};

    m_offset = static_cast<std::size_t>(get<std::byte>(aligned) -
                                        get<std::byte>(m_memory)) +
               size;
  ...
  void deallocate(void* pointer) override {
    ...
    const auto* header = get<Header>(get<std::byte>(pointer) - sizeof(Header));

    m_offset = static_cast<std::size_t>(get<std::byte>(pointer) -
                                        get<std::byte>(m_memory)) -
               header->size;
  }
...
public:
  ListAllocator(std::size_t page_size, std::size_t block_size)
    : m_block_size(align_up(std::max<std::size_t>(block_size, sizeof(Node)),
                            kDefaultAlignment))
    , m_page_size(align_up(std::max<std::size_t>(page_size, m_block_size),
                           m_block_size)) {}
...
public:
  ...
  void* allocate(std::size_t size,
    ...
    const std::size_t payload_size = std::max(size, minimum_payload_size());
    ...
    if (current->size >= payload_size + minimum_total_block_size()) {
      auto* next_block =
        get<std::byte>(current) + sizeof(Header) + payload_size;

      auto* next_node = ::new (static_cast<void*>(next_block))
        Node{current->size - payload_size - sizeof(Header), current->next};
      ...
      auto* header = ::new (static_cast<void*>(current)) Header{payload_size};
    ...
    auto* header = ::new (static_cast<void*>(current)) Header{current->size};
  ...
  void deallocate(void* pointer) override {
    ...
    auto* node = ::new (static_cast<void*>(header)) Node{header->size, nullptr};
...
private:
  static constexpr std::size_t minimum_payload_size() noexcept {
    return sizeof(Node) > sizeof(Header) ? sizeof(Node) - sizeof(Header) : 1;
  }
  ...
  static void merge_with_next(Node* node) noexcept {
    ...
    auto* node_end = get<std::byte>(node) + sizeof(Header) + node->size;
