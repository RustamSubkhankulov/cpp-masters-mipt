...
private:
  ...
  public:
    ...
    Iterator& operator++() {
      for (difference_type skipped = 0; skipped < step_ && current_ != end_;
           ++skipped) {
        ++current_;
      }
      return *this;
...
TEST(FibonacciView, WorksWithStandardViews) {
  ...
  auto values =
    Fibonacci(fibonacci_count) | std::views::drop(dropped_count) |
    std::views::filter([](std::uint64_t value) { return value > threshold; });
