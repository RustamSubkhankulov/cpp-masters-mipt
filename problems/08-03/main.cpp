...
struct FloatFormat final {
    ...
    static constexpr unsigned int exponent_all_ones =
        (1U << exponent_bits) - 1U;
    static constexpr unsigned int fraction_mask =
        (1U << fraction_bits) - 1U;
    static constexpr unsigned int sign_mask =
        1U << (unsigned_bit_count - 1U);
};
...
[[nodiscard]] std::optional<int> FindMostSignificantBitIndex(unsigned int value) {
    if (value == 0U) {
        return std::nullopt;
    }
...
TEST(IntBinaryLogarithmFloorTest, HandlesPositiveIntegers) {
    ...
    for (const TestCase& test_case : test_cases) {
        ExpectOptionalValue(
            BinaryLogarithmFloor(test_case.input),
            test_case.expected
        );
    }
}
...
TEST(IntBinaryLogarithmFloorTest, RejectsNonPositiveIntegers) {
    ...
    EXPECT_FALSE(BinaryLogarithmFloor(std::numeric_limits<int>::min()).has_value());
}
...
TEST(DemoExamples, ShowsTypicalUsageExamples) {
    ...
    ExpectOptionalValue(
        BinaryLogarithmFloor(std::numeric_limits<float>::denorm_min()),
        -149
    );
}
