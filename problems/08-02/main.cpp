...
public:
  ...
  Integer& operator*=(const Integer& other) {
    ...
    std::vector<std::uint32_t> result(digits_.size() + other.digits_.size(),
                                      0U);
    for (std::size_t left = 0; left < digits_.size(); ++left) {
      std::uint64_t carry = 0U;
      for (std::size_t right = 0; right < other.digits_.size() || carry != 0U;
           ++right) {
  ...
  friend std::ostream& operator<<(std::ostream& output, const Integer& value) {
    ...
    for (std::size_t index = value.digits_.size() - 1; index > 0; --index) {
      output << std::setw(static_cast<int>(kBaseDigits))
             << value.digits_[index - 1];
    }
...
private:
  ...
  void parse(std::string_view text) {
    ...
    for (std::size_t block_end = text.size(); block_end > offset;) {
      ...
      const std::size_t block_size =
        available >= kBaseDigits ? kBaseDigits : available;
  ...
  static void add_abs_to(Integer& left, const Integer& right) {
    const std::size_t result_size =
      std::max(left.digits_.size(), right.digits_.size());
  ...
  static void sub_abs_from(Integer& left, const Integer& right) {
    ...
    for (std::size_t index = 0; index < left.digits_.size(); ++index) {
      const std::int64_t current =
        static_cast<std::int64_t>(left.digits_[index]) -
        (index < right.digits_.size()
           ? static_cast<std::int64_t>(right.digits_[index])
           : 0) -
        borrow;
      if (current < 0) {
        left.digits_[index] = static_cast<std::uint32_t>(
  ...
  static Integer multiply_abs_by_digit(const Integer& value,
                                       std::uint32_t digit) {
    ...
    for (std::size_t index = 0; index < value.digits_.size(); ++index) {
      const std::uint64_t product =
        static_cast<std::uint64_t>(value.digits_[index]) * digit + carry;
  ...
  static std::pair<Integer, Integer> divmod(const Integer& dividend,
...
TEST(IntegerTest, ArithmeticFromExample815) {
  ...
  EXPECT_EQ((x *= y).str(),
            "+246913580246913580246913580246908641975308641975308641975308642");
  EXPECT_EQ((x /= y).str(), "+11111111111111111111111111111111");
...
TEST(IntegerTest, ExceptionalCases) {
  EXPECT_THROW(static_cast<void>(Integer("10") / Integer(0)),
               std::domain_error);
  EXPECT_THROW(static_cast<void>(Integer("10") % Integer(0)),
               std::domain_error);
