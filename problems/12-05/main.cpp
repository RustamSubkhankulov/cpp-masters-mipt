...
public:
    ...
    explicit Token(double value) : kind_(Kind::number), value_(value) {}
...
class TokenStream {
public:
    explicit TokenStream(std::string input) : input_(std::move(input)) {}
    ...
    auto get() -> Token
    {
        ...
        switch (symbol) {
            case '+': return Token(Token::Kind::plus);
            case '-': return Token(Token::Kind::minus);
            case '*': return Token(Token::Kind::multiply);
            case '/': return Token(Token::Kind::divide);
            case '%': return Token(Token::Kind::remainder);
            case '^': return Token(Token::Kind::power);
            case '!': return Token(Token::Kind::factorial);
            case '=': return Token(Token::Kind::assign);
            ...
            case '[': return Token(Token::Kind::left_square);
            case ']': return Token(Token::Kind::right_square);
            case '{': return Token(Token::Kind::left_brace);
            case '}': return Token(Token::Kind::right_brace);
            default: break;
        }
...
private:
    ...
    auto bracketed_expression(TokenStream& stream, Token::Kind closing_kind) -> double
    {
        double value = expression(stream);
...
TEST(CalculatorRemainder, ComputesRemainder)
{
    ...
    EXPECT_THROW(static_cast<void>(calculator.evaluate("5 % 0")), std::runtime_error);
}
...
TEST(CalculatorFactorial, ComputesPostfixFactorial)
{
    ...
    EXPECT_THROW(static_cast<void>(calculator.evaluate("(-3)!")), std::runtime_error);
    EXPECT_THROW(static_cast<void>(calculator.evaluate("2.5!")), std::runtime_error);
}
...
TEST(CalculatorInputValidation, RejectsInvalidInput)
{
    ...
    EXPECT_THROW(static_cast<void>(calculator.evaluate("(1 + 2]")), std::runtime_error);
    EXPECT_THROW(static_cast<void>(calculator.evaluate("unknown + 1")), std::runtime_error);
    EXPECT_THROW(static_cast<void>(calculator.evaluate("1 + 2 3")), std::runtime_error);
    EXPECT_THROW(static_cast<void>(calculator.evaluate("set = 3")), std::runtime_error);
}
...
TEST(CalculatorDemoExamples, HandlesCompoundExamples)
{
    ...
    EXPECT_DOUBLE_EQ(calculator.evaluate("(5 - 2) / {6 * (8 + 14)}"), 3.0 / 132.0);
