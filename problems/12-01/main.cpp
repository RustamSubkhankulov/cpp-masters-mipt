...
std::string ExtractRubAmountText(std::string_view input_text) {
  ...
  if (HasTrailingRubCode(trimmed_text)) {
    return std::string(
      TrimAscii(trimmed_text.substr(0, trimmed_text.size() - code.size())));
  }
...
std::optional<LocalePair> TryCreateRequiredLocales() {
  try {
    return LocalePair{std::locale(std::string(RussianLocaleName())),
...
TEST(RubToUsdConversionTest, ConvertsInputWithLeadingRubCode) {
  ...
  const std::string output_text =
    ConvertRubToUsd(input_text, rate, locales->rub_locale, locales->usd_locale);
...
TEST(RubToUsdConversionTest, RejectsMissingRubCode) {
  ...
  EXPECT_THROW(ConvertRubToUsd(input_text, rate, std::locale::classic(),
                               std::locale::classic()),
