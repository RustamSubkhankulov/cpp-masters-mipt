 ...
 * include/boost/graph/adjacency_matrix.hpp:1113:68: warning: unused parameter 'g' [-Wunused-parameter]
 *  1113 |     static type get_nonconst(adjacency_matrix< D, VP, EP, GP, A >& g, Tag tag)
 *       |                                                                    ^
 *  include/boost/graph/adjacency_matrix.hpp:1119:53: warning: unused parameter 'g' [-Wunused-parameter]
 *  1119 |         const adjacency_matrix< D, VP, EP, GP, A >& g, Tag tag)
 *       |                                                     ^
 *  include/boost/graph/adjacency_matrix.hpp:1125:47: warning: unused parameter 'g' [-Wunused-parameter]
 *  1125 |         adjacency_matrix< D, VP, EP, GP, A >& g, Tag tag, edge_descriptor e)
 *       |                                               ^
 * include/boost/graph/adjacency_matrix.hpp:1132:53: warning: unused parameter 'g' [-Wunused-parameter]
...
GeneratedGraph MakeCompleteGraph(std::default_random_engine& generator)
{
    ...
    std::vector<std::vector<int>> weights(vertices, std::vector<int>(vertices, 0));
    std::uniform_int_distribution<int> distribution(MinEdgeWeight(), MaxEdgeWeight());

    for (std::size_t from = 0U; from < vertices; ++from) {
        for (std::size_t to = from + 1U; to < vertices; ++to) {
            const int weight = distribution(generator);
            const auto edge = boost::add_edge(from, to, EdgeWeightProperty(weight), graph);
            if (!edge.second) {
...
void PrintWeightMatrix(const std::vector<std::vector<int>>& weights, std::ostream& output)
{
...
GeneratedGraph MakeKnownGraph()
{
    ...
    std::vector<std::vector<int>> weights(VertexCount(), std::vector<int>(VertexCount(), 0));
...
bool AreAdjacentInCycle(const VertexPath& cycle,
                        VertexIndex first_vertex,
                        VertexIndex second_vertex)
{
    ...
    const std::size_t first_index = static_cast<std::size_t>(
        std::distance(cycle.begin(), first));
    const std::size_t second_index = static_cast<std::size_t>(
        std::distance(cycle.begin(), second));
