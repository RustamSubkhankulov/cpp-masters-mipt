...
std::string MutateString(
    ...
    for (char& symbol : child) {
        if (mutation_distribution(engine) < mutation_probability) {
            symbol = GenerateRandomDifferentLowercaseLetter(
                symbol,
                engine,
                letter_distribution);
        }
...
std::vector<std::string> GeneratePopulation(
    const std::string& parent,
...
std::pair<std::string, std::size_t> SelectBestCandidate(
    ...
    for (const std::string& candidate : population) {
        const std::size_t candidate_distance =
            HammingDistance(candidate, target);
...
TEST(AlgorithmTest, ReachesBookTargetWithDeterministicSeed) {
    ...
    const dawkins::RunResult result =
        dawkins::Run(settings, engine, std::cout);
