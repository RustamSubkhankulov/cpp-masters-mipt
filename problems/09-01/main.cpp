...
#else
#define trace() \
    [[maybe_unused]] ::Tracer TRACER_CONCAT(tracer_instance_at_line_, __LINE__){std::source_location::current()}
#endif
...
#else
    const std::size_t root_enter = FindTraceLinePosition(output, "enter", "ExampleRoot");
    const std::size_t leaf_enter = FindTraceLinePosition(output, "enter", "ExampleLeaf");
    const std::size_t leaf_leave = FindTraceLinePosition(output, "leave", "ExampleLeaf");
    const std::size_t root_leave = FindTraceLinePosition(output, "leave", "ExampleRoot");
